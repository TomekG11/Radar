...
public class SettingsScreen extends Screen {
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        int gX = guiX;
        int gY = guiY + (int)((1.0F - ease) * 20.0F);
        ...
        for (int i = 0; i < presets.length; i++) {
            ModSettings.ColorPreset preset = presets[i];
            ...
            renderBtn(ctx, label, style, bX, bY, btnW, btnH, hitTest(mouseX, mouseY, bX, bY, btnW, btnH));
        }

        int rowCount = (int) Math.ceil((double) presets.length / 4.0D);
        ...
        ctx.drawTextWithShadow(textRenderer, Text.literal(descSet), gX + 14, y, -7829351);
        y += 12;
        String descElytra = ModSettings.showElytra ? "§aWykrywaj graczy z elytrem: WŁ" : "§7Wykrywaj graczy z elytrem: WYŁ";
        ctx.drawTextWithShadow(textRenderer, Text.literal(descElytra), gX + 14, y, -7829351);
        y += 14;
        int fBtnW = 245;
        ...
        ctx.drawTextWithShadow(textRenderer, Text.literal(perfDesc), gX + 14, y, -7829351);
        y += 12;
        String tpaDesc = ModSettings.closeOnTpa ? "§aZamknij GUI po akceptacji TPA: WŁ" : "§7Zamknij GUI po akceptacji TPA: WYŁ";
        ctx.drawTextWithShadow(textRenderer, Text.literal(tpaDesc), gX + 14, y, -7829351);
        y += 14;
        int gBtnW = 245;
        ...
        int backY = gY + GUI_H - 40;
        renderBtn(ctx, "← Powrót", GuiButton.Style.WHITE, backX, backY, backW, 22, hitTest(mouseX, mouseY, backX, backY, backW, 22));
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        for (int i = 0; i < presets.length; i++) {
            ...
            int bX = gX + 20 + col * (btnW + gap);
            int bY = y + row * (btnH + gap);
            if (hitTest(mx, my, bX, bY, btnW, btnH)) {
                ModSettings.setColor(presets[i]);
                return true;
            }
        }
        ...
        y += 26;
        int fBtnW = 245;
        if (hitTest(mx, my, gX + 20, y, fBtnW, btnH)) {
            ModSettings.fullSetOnly = !ModSettings.fullSetOnly;
            return true;
        }
        if (hitTest(mx, my, gX + 20 + fBtnW + 10, y, fBtnW, btnH)) {
        ...
        y += 26;
        int gBtnW = 245;
        if (hitTest(mx, my, gX + 20, y, gBtnW, btnH)) {
            ModSettings.performanceMode = !ModSettings.performanceMode;
            return true;
        }
        if (hitTest(mx, my, gX + 20 + gBtnW + 10, y, gBtnW, btnH)) {
        ...
        int blBtnW = 220;
        int blBtnX = gX + 270 - blBtnW / 2;
        if (hitTest(mx, my, blBtnX, y, blBtnW, btnH)) {
            MinecraftClient.getInstance().setScreen(new BlacklistScreen(this));
            return true;
        }
        ...
        int backX = gX + 270 - backW / 2;
        int backY = gY + GUI_H - 40;
        if (hitTest(mx, my, backX, backY, backW, 22)) {
